...
const AllowancesListContainer = ({
  ...
  const allowancesByTokenId = useAppSelector((state: RootState) => {
    ...
    if (showZeroAllowances && addressFilter === '') {
      ...
      // apply filter
      allowanceIds = state.allowances.allowanceIdsByOwnerId[ownerId].filter(
        (allowanceId) => {
          ...
          if (!showZeroAllowances) {
            ...
            const isZeroAllowance =
              allowanceValue.state === QueryStates.QUERY_STATE_COMPLETE &&
          ...
          if (addressFilter) {
            ...
            if (tokenContract) {
              const tokenContractAddress =
                state.addresses.addressesById[tokenContract.addressId]
              const matchedFilter =
    ...
    return _.sortBy(groupedAllowances, [
      function (allowanceArray) {
        const tokenContract =
          state.tokenContracts.contractsById[allowanceArray[0].tokenContractId]
